...
void ZZInstallRuntimeFiltering(void) {
    @synchronized (ZZRuntimeLock()) {
        ...
        if (!classes.count) {
            ZZFilterDebugWrite(@"[ZZFilterUI] v27 known listing controllers not loaded yet");
            return;
        }
        ...
        ZZFilterDebugWrite(@"[ZZFilterUI] v27 targeted hooks installed=%lu classes=%lu", (unsigned long)ZZRuntimeHooks, (unsigned long)classes.count);
    }
}
...
NSString *ZZRuntimeDiagnosticSummary(void) {
    return [NSString stringWithFormat:@"v27 targeted listing hooks=%lu", (unsigned long)ZZRuntimeHooks];
}
