...
void ZZInstallRuntimeFiltering(void) {
    @synchronized (ZZRuntimeLock()) {
        ...
        if (!classes.count) {
            ZZFilterDebugWrite(@"[ZZFilterUI] v32 known listing controllers not loaded yet");
            return;
        }
        ...
        ZZFilterDebugWrite(@"[ZZFilterUI] v32 targeted hooks installed=%lu classes=%lu", (unsigned long)ZZRuntimeHooks, (unsigned long)classes.count);
    }
}
...
NSString *ZZRuntimeDiagnosticSummary(void) {
    return [NSString stringWithFormat:@"v32 targeted listing hooks=%lu", (unsigned long)ZZRuntimeHooks];
}
